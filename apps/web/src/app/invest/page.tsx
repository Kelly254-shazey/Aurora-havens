...
export const metadata: Metadata = {
  title: 'Invest',
  description: 'Explore premium real estate investment opportunities across Africa with competitive ROI. Start investing with Aurora Havens today.',
};
          ...
          <div className="text-center mb-16">
           ...
           <div className="grid md:grid-cols-3 gap-4 sm:gap-8">
            {STEPS.map((step, index) => (
              <div key={index} className="relative text-center p-8 rounded-2xl border border-gray-100 hover:border-gold-500/30 hover:shadow-xl hover:shadow-gold-500/5 transition-all duration-500 group">
                <div className="absolute -top-4 left-8 bg-dark-900 text-gold-400 font-display font-bold text-sm px-3 py-1 rounded-lg shadow-lg">
                  {step.step}
                </div>
            ...
            {OPPORTUNITIES.map((opp, index) => (
              <div key={index} className="group bg-dark-900 rounded-2xl overflow-hidden border border-gold-500/20 hover:border-gold-500/40 hover:shadow-2xl hover:shadow-gold-500/10 transition-all duration-500">
                <div className="relative h-48 bg-gradient-to-br from-dark-800 to-dark-900 flex items-center justify-center">
                  ...
                  <span className="absolute top-4 right-4 bg-dark-900/80 backdrop-blur-sm text-white text-xs font-medium px-2.5 py-1 rounded-lg border border-white/10">
                    {opp.type}
                  </span>
                </div>
                <div className="p-6">
      ...
      <section className="py-16 sm:py-20 lg:py-24 bg-white">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="space-y-4">
            {FAQS.map((faq, index) => (
              <div key={index} className="border border-gray-200 rounded-2xl overflow-hidden hover:border-gold-500/30 transition-colors">
